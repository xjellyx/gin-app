...
export function useTable<A extends NaiveUI.TableApiFn>(config: NaiveUI.NaiveTableConfig<A>) {
  ...
  const {
    ...
    transformer: res => {
      const { records = [], pagination = { pageNum: 1, pageSize: 10, total: 0 } } = res.data || {};

      const recordsWithIndex = records.map((item, index) => {
        return {
          ...item,
          index: (pagination.pageNum - 1) * pagination.pageSize + index + 1
        };
      });
  ...
  const pagination: PaginationProps = reactive({
    ...
    onUpdatePage: async (pageNum: number) => {
