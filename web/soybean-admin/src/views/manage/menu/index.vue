...
import SysApiAuthModal from './modules/api-auth-modal.vue';
...
const { bool: sysApiAuthVisible, setTrue: openSysApiAuthModal } = useBoolean();
const { columns, columnChecks, data, loading, pagination, getData } = useTable({
  ...
  columns: () => [
    ...
    },
    {
      ...
      width: 60,
      render: row => {
        const icon = Number(row.iconType) === 1 ? row.icon : undefined;

        const localIcon = Number(row.iconType) === 2 ? row.icon : undefined;
