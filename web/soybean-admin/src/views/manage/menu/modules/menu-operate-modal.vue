...
watch(model, () => {
  if (Number(model.menuType) === 3) {
    model.i18nKey = `route.${model.menuName}` as App.I18n.I18nKey;
  }
});
      ...
      <NForm ref="formRef" :model="model" :rules="rules" label-placement="left" :label-width="100">
        <NGrid responsive="screen" item-responsive>
          ...
          <NFormItemGi span="24 m:12" :label="$t('page.manage.menu.icon')" path="icon">
            <template v-if="Number(model.iconType) === 1">
            ...
            <template v-if="Number(model.iconType) === 2">
              <NSelect
